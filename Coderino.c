...
*/
//Converts the state of a button to a boolean expression. Ex. Pressed = true, Not Pressed = fasle
...
int getPotential(int potentiometer){
	int potentioMeter;
...
int applyThingsNStuff(int joyValue, int threshold, int throttle){
	int trueValue;
	if(toPercent(joyValue, 124) < throttle){
...
int doPassiveClaw(int button, int maxSpeed, int potentiometer, int closeValue, int openValue){
	int motorValue;
	if(isPressed(button) && getPotential(potentiometer) < openValue){
		motorValue = maxSpeed;
		}else if(isPressed(button) == false && getPotential(potentiometer) > closeValue){
